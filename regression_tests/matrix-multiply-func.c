...
/* fpsanitizer: A micro benchmark to test performance overheads with
   frequent mallocs and frees and to measure the cost of function
   calls and return with FP arguments */


float mult_ind(float a, float b){
  return a * b;
}

float mult_add(float a, float b){
  return a + b;
}
...
float** matrix_mult(float** A, float** B,
  ...
  for(int i =0; i < m1; i++){
	...
	float t1 = mult_ind(A[i][k], B[k][j]);
	result[i][j] = mult_add(result[i][j], t1);
      }
    }
  }
  return result;
